defmodule GrasstubeWeb.UserAuth do
  use GrasstubeWeb, :verified_routes
  ...
  alias Grasstube.Accounts.Scope

  # Make the remember me cookie valid for 14 days. This should match
  # the session validity setting in UserToken.
  @max_cookie_age_in_days 14
  @remember_me_cookie "_grasstube_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      GrasstubeWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule GrasstubeWeb.PageLive do
        use GrasstubeWeb, :live_view

        on_mount {GrasstubeWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{GrasstubeWeb.UserAuth, :require_authenticated}] do
  ...
  defp mount_current_scope(socket, session) do
    Phoenix.Component.assign_new(socket, :current_scope, fn ->
      ...
      %{Scope.for_user(user) | country_code: session["country_code"]}
    end)
  end
  ...
  def fetch_geo(conn, _opts) do
    ip =
      conn.req_headers
      |> Enum.filter(&(elem(&1, 0) == "x-real-ip"))
      |> Enum.at(0)
      |> elem(1)
